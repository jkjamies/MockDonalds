package com.jkjamies.sampleplatter.konsist.architecture
...
class ForbiddenPatternsTest : BehaviorSpec({
    ...
    Given("CenterPost only — no raw coroutine scope or hardcoded dispatchers") {
        ...
        Then("feature modules should not directly use launch or async") {
            ...
            val violators = featureAndPresenterFiles
                ...
                // so `name.endsWith("Ui.kt")` matched nothing and this carve-out never applied.
                ...
                .filter { !it.nameWithExtension.endsWith("Ui.kt") }
                .flatMap { file ->
                    file.imports
                        .filter {
                    ...
                    file.imports
                        .filter {
                            it.name.contains(".androidApp.") ||
                                it.name.contains("com.jkjamies.sampleplatter.android.")
                        }
                        .map { "  ${file.name}: ${it.name}" }
                }
    ...
    Given("Compose runtime, not Compose UI, in shared code") {
        ...
        //
        // Compose UI belongs in androidMain. The `sampleplatter.kmp.presentation` convention
