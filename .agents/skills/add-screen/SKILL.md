...
| `{Screen}Presenter.kt` | impl/presentation/src/commonMain | @CircuitInject + @Inject + @Composable. Collect with `collectContentAsState()` and map through `dataOrNull` / `isLoading` / `errorOrNull` — see `.agents/standards/strata.md` → "Two collection surfaces" |
