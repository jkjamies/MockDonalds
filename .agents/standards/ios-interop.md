...
- **SPM package** colocated in the core module under `{module}/impl/{PackageName}/` (where `PackageName` matches the SPM library name, e.g., `RemoteConfigBridge`) — owns the vendor SDK dependency and all vendor-specific work (SDK init, variation reads, listener registration). Exposes a Swift-native public API (e.g., `HarnessClient`). **Does not import `ComposeApp`.**
...
- Add the local package to `iosApp/project.yml` under `packages:` (`path: ../core/{module}/impl/{PackageName}` — directory basename must match the SPM library name so the Xcode navigator displays it correctly) and list its product under `targets.iosApp.dependencies`.
