...
iosApp/iosAppTests/UIComponent/{Feature}/
...
The `@CircuitInject` macro tags the view as the renderer for the given Screen → UiState pair. `CircuitFactoryRegistry` (build-time codegen) discovers it during the next build and emits the entry in `iosApp/iosApp/Generated/GeneratedCircuitFactories.swift`. **No `AppDelegate.swift` edit is needed.** See `.agents/standards/ios-interop.md` for macro/registry details.
...
**iOS UI factory** -- AUTOMATIC. The `@CircuitInject({Feature}Screen.self, {Feature}UiState.self)` annotation on the SwiftUI view is picked up by `CircuitFactoryRegistry` (Run Script Phase) and added to `iosApp/iosApp/Generated/GeneratedCircuitFactories.swift` on the next build. No `AppDelegate.swift` edit needed.
...
| SwiftUI View | `iosApp/Features/` | iOS-only, pure state function. `@CircuitInject(Screen.self, UiState.self)` annotation drives factory codegen. |
| UI Registration | `iosApp/iosApp/Generated/GeneratedCircuitFactories.swift` | Generated by `CircuitFactoryRegistry` Run Script Phase from `@CircuitInject` annotations — never hand-edited |
...
10. `iosApp/Features/{Name}/` -- SwiftUI view with `@CircuitInject(Screen.self, UiState.self)`
11. `iosApp/iosAppTests/UIComponent/{Name}/` -- StateRobot, ViewRobot, ViewTest (Swift Testing + ViewInspector)
12. `App.kt` -- screen route mapping
13. iOS UI factory wiring -- AUTOMATIC (`@CircuitInject` → `CircuitFactoryRegistry` → `GeneratedCircuitFactories.swift`)
