...
class StrataContentStateTest : BehaviorSpec({

    Given("an interactor whose observable emits values") {
        val backing = MutableStateFlow("first")
        val interactor = object : StrataSubjectInteractor<Unit, String>() {
        ...
        When("contentState is collected") {
            Then("it emits Loading before the first value") {
                ...
                interactor.contentState.test {
                    awaitItem() shouldBe StrataContentState.Loading
                    awaitItem() shouldBe StrataContentState.Content("first")
                }
            }
            ...
            Then("subsequent values arrive as Content") {
                ...
                interactor.contentState.test {
                    ...
                    awaitItem() shouldBe StrataContentState.Content("second")
                }
            }
        }
    }
        ...
        When("contentState is collected") {
            Then("the failure surfaces as an Error state rather than propagating") {
                ...
                interactor.contentState.test {
                    ...
                    val error = awaitItem().shouldBeInstanceOf<StrataContentState.Error>()
                    error.error.shouldBeInstanceOf<StrataExecutionException>()
    ...
    Given("an interactor whose observable emits then throws") {
        ...
        When("contentState is collected") {
            Then("the emitted value is kept and the failure follows it") {
                ...
                interactor.contentState.test {
                    awaitItem() shouldBe StrataContentState.Loading
                    awaitItem() shouldBe StrataContentState.Content("partial")
                    awaitItem().shouldBeInstanceOf<StrataContentState.Error>()
                    // contentState never completes — paramState is an infinite SharedFlow,
                    // and the catch is inside flatMapLatest so only the inner flow ends.
                    cancelAndIgnoreRemainingEvents()
                }
    ...
    Given("a StrataException thrown by the observable") {
        val original = StrataTimeoutException(kotlin.time.Duration.ZERO, IllegalStateException("t"))
        val interactor = object : StrataSubjectInteractor<Unit, String>() {
        ...
        When("contentState is collected") {
            Then("it is surfaced as-is rather than being wrapped again") {
                ...
                interactor.contentState.test {
                    awaitItem() shouldBe StrataContentState.Loading
                    awaitItem() shouldBe StrataContentState.Error(original)
                    // contentState never completes — paramState is an infinite SharedFlow,
                    // and the catch is inside flatMapLatest so only the inner flow ends.
                    cancelAndIgnoreRemainingEvents()
                }
        ...
        When("retry is called after the failure") {
            Then("the stream restarts and reaches Content") {
                ...
                interactor.contentState.test {
                    ...
                    interactor.retry()
                    awaitItem() shouldBe StrataContentState.Loading
                    awaitItem() shouldBe StrataContentState.Content("recovered")
                    cancelAndIgnoreRemainingEvents()
                }
                attempts shouldBe 2
            }
    ...
    Given("content state accessors") {
        When("inspecting each state") {
            Then("dataOrNull, isLoading and errorOrNull report the right case") {
                val loading: StrataContentState<String> = StrataContentState.Loading
                val content: StrataContentState<String> = StrataContentState.Content("v")
                val error: StrataContentState<String> =
                    StrataContentState.Error(StrataExecutionException(IllegalStateException("e")))
                ...
                error.errorOrNull.shouldBeInstanceOf<StrataExecutionException>()
            }
        }
    }
})
