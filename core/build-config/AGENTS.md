...
Split into three leaf modules matching `core:remote-config`, `core:analytics`, `core:auth`:
...
| **test** | `:core:build-config:test` | `FakeAppBuildConfig` (**auto-generated** by `:build-tooling:ksp-fake-app-build-config`; `@ContributesBinding(AppScope::class)`; every property is an `override var` with a type-empty default) | Test graphs in modules that swap out `AppBuildConfig` |
...
| `BuildConfigField` | `api` | Data class (`name`, `value`, `group: Group`) plus enumeration extension `fun AppBuildConfig.asFields(): List<BuildConfigField>`. Powers the debug-menu build-config viewer and any future read-only dumpers. `Group` is `Identity`, `Urls`, or `Localization`. **The list is auto-generated by `BuildConfigRegistryProcessor` (KSP)** — `asFields()` is a pure delegation to the generated `generatedFields()`. Add a field by annotating the new `AppBuildConfig` property with `@DebugConfigField(Group.…)`. |
| `DebugConfigField` | `api` | Property-level annotation consumed by the KSP processor in `:build-tooling:ksp-build-config-registry`. Must be present on every `AppBuildConfig` property — the processor fails the build with a precise error if any is missing, and `BuildConfigCoverageTest` catches it earlier at the Konsist layer. |
...
| `FakeAppBuildConfig` | `test` | Test binding — **KSP-generated** from `AppBuildConfig` by `:build-tooling:ksp-fake-app-build-config`. Every property is an `override var` with a type-empty default (`""` / `0` / `false`). Tests construct `FakeAppBuildConfig()` and mutate only the fields they care about. Never hand-written — `BuildConfigRegistryIntegrityTest` fails if a file at this name appears outside `build/generated/`. |
...
3. Expose it on `AppBuildConfig` in `api/src/commonMain/kotlin/…/AppBuildConfig.kt` **annotated with `@DebugConfigField(Group.…)`**. The KSP registry processor reads this annotation and emits the debug-menu row automatically — no edit to `BuildConfigField.kt` is needed.
4. Override it in `AppBuildConfigImpl` in `impl/src/commonMain/kotlin/…/AppBuildConfigImpl.kt`.
5. Consume via `AppBuildConfig.fieldName` (injected via Metro).

Both the debug-menu registry (`BuildConfigField.asFields()`) and the test double (`FakeAppBuildConfig`) are generated at compile time from the `AppBuildConfig` interface — engineers never touch either. `BuildConfigCoverageTest` and `BuildConfigRegistryIntegrityTest` fail the build if the generators are bypassed, shadowed, or misused.
