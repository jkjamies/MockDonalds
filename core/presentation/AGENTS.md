...
Cross-cutting Compose-specific code: extensions over otherwise Compose-free core modules **and standalone UI primitives** that don't belong inside a single feature. Lets non-presentation core modules (`core:strata`, `core:remote-config:api`, future `core:analytics:api`, etc.) stay free of Compose runtime while still offering Composable extension points to presentation layers, and provides reusable UI primitives (e.g., embedded WebView) shared across multiple features.

This module is the only Compose-aware seam over otherwise plain-Kotlin core APIs. Convention plugin `sampleplatter.kmp.presentation` auto-wires it into every feature `impl/presentation` module, so feature presenters get `rememberFlag`, `rememberConfig`, `rememberStrata`, `StrataSubjectInteractor.collectAsState`, and `WebViewContent` automatically — no per-feature dep declaration required.
...
core/presentation/
  ...
  src/commonMain/kotlin/com/jkjamies/sampleplatter/core/presentation/
    strata/
      RememberStrata.kt            @Composable rememberStrata(dispatchers): Strata
      CollectAsState.kt                @Composable StrataSubjectInteractor<Unit, T>.collectAsState(initial)
    remoteconfig/
...
The package layout mirrors the source core module (`strata/`, `remoteconfig/`) for extensions, while standalone primitives (`webview/`) live in `androidMain` because they render Android-native UI. The iOS sibling for cross-cutting SwiftUI primitives lives at `iosApp/iosApp/Presentation/` (see `iosApp/iosApp/Presentation/AGENTS.md`).

## Public API

| Type | Source set | Subpackage | Description |
|------|-----------|-----------|-------------|
| `rememberStrata(dispatchers)` | commonMain | `strata` | Compose-scoped factory for `Strata`; survives recomposition, cancels with the host |
| `StrataSubjectInteractor<Unit, T>.collectAsState(initial)` | commonMain | `strata` | Compose extension that triggers the interactor with `Unit` and collects its `Flow<T>` as `State<T?>` |
...
- This module depends only on `core:strata` and `core:remote-config:api` today. Any future Composable extension over a new core type adds an `api(...)` dep on that core's `:api` module — never on its `:impl`.
...
- The seam between Compose-aware and Compose-free is **the source core module's api/impl boundary** — `core:strata` and `core:remote-config:api` stay Compose-free; their Composable extensions live here. `ComposeIsolationTest` enforces the rule.
