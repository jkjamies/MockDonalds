...
### Buttons
- **Primary:** Gradient fill (`primary_container` to `#930003`), `roundedness-1` (subtle roundedness), `title-sm` (Manrope Bold).
...
- **Execution:** Use `normal` (spacing scale `2`) of vertical whitespace to separate items. For lists, use a alternating tonal shift (e.g., Row 1 is `surface`, Row 2 is `surface_container_low`).
- **Product Cards:** Use a subtle `roundedness-1` (subtle roundedness) for corner radius. The image should occupy the top 70% of the card, bleeding to the edges.
