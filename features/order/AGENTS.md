...
- **Akamai exception**: Spoonacular is a third-party reference integration and does **not** route through Akamai. The "Akamai required in all envs" project rule applies to first-party SamplePlatter backend services only. Future networked features that consume first-party APIs must route through Akamai.
